...
describe("draw-utils", () => {
  ...
  describe("isDrawTool / getToolStrokeWidth", () => {
    ...
    it("borracha usa traço maior que o pincel no mesmo preset", () => {
      ...
      expect(getToolStrokeWidth("pen", "fine")).toBe(getStrokeWidth("fine"));
    });
  });
  ...
  describe("getStrokeWidth", () => {
    ...
    it("mantém ordem crescente fino < médio < grosso", () => {
      expect(getStrokeWidth("fine")).toBeLessThan(getStrokeWidth("medium"));
      expect(getStrokeWidth("medium")).toBeLessThan(getStrokeWidth("thick"));
    });
  });
