...
function animationDelay(date: string, rangeStart: string) {
  ...
  const days = Math.round(
    (Date.parse(`${date}T00:00:00Z`) - Date.parse(`${rangeStart}T00:00:00Z`)) /
      DAY_MS,
  );
      ...
      {mounted ? (
        <GitHubCalendar
          ...
          tooltips={{
            activity: {
              text: (activity) =>
                formatCommitTooltip(activity, t, labels.monthsLong),
            },
          }}
          renderBlock={(block, activity) => {
            const delay = animationDelay(activity.date, rangeStartRef.current);
            ...
            const label =
              activity.count > 99 ? "99+" : String(activity.count);
            ...
            return (
              <g>
                ...
                <text
                  ...
                  fill={activity.level >= 3 ? "#fff" : "var(--foreground)"}
                  style={{
